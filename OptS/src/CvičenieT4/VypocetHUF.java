...
public class VypocetHUF {
    ...
    public int objUFLP_1(int m, int n, int[] y, int[] f, int[] b, int[][] d) {
        ...
        for (j = 0; j < m; j++) {  //pre každého zákazníka
            ...
            for (i = 0; i < n; i++) { // pre každý sklad
                if (y[i] == 1) { // ak je sklad vybratý tak ho použijem na rozhodovanie
                    cand = d[i][j];
        ...
        for (i = 0; i < n; i++) {
            sum += f[i] * y[i]; // pridaj fixné náklady na aktívne sklady
        }
     ...
     * @return hodnotu účelovej funkcie
     */
    public int objUFLP_2(int n, int m, int noy, Integer[]zy, int[]f, int[]b, int[][]d){
        int i, j; // indexi
        int sum; // hodnota ucelovej funkcie
        ...
        for (j = 0; j < m; j++) {  //pre každého zákazníka
            mind = Integer.MAX_VALUE;
            for (i = 0; i < noy; i++) { // pre každý sklad
                cand = d[zy[i]][j];
                mind = Math.min(mind, cand);
            }
            sum += mind * b[j]; // náklady sú dané súčinom vzdialenosti a požiadavky
        ...
        for (i = 0; i < noy; i++) {
            sum += f[zy[i]]; // pridaj fixné náklady
        }
